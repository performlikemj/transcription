...
class DictationApp(rumps.App):
    def __init__(self):
        ...
        self.hotkey_string = "<ctrl>+<alt>+<space>"
        ...
        self.update_menu_state() # Initial state for title: "ASR Init..."

        self._debug_id_counter = 0 # For unique log IDs

        rumps.notification("Dictation App", "Initializing", "Loading ASR model in background...")
    ...
    def _handle_asr_service_result(self, result_payload, error_payload):
        ...
        if result_payload == "MODEL_LOADED_SUCCESSFULLY":
            ...
            if result_payload is None or (isinstance(result_payload, str) and result_payload.strip() == ""):
                ...
                try:
                    if error_obj:
                        ...
                        if transcribed_text == self._last_transcribed_text:
                            ...
                            print("------------------------")
                            if self.text_insertion_service.insert_text(transcribed_text):
    ...
    def _activate_dictation_main(self):
        ...
        if not self.dictation_active:
            ...
            self.audio_manager.set_chunk_callback(self.asr_service.process_audio_chunk)
            ...
            if self.audio_manager.start_recording(f"from_activate_{log_id}"):
                ...
                rumps.alert("Audio Error", "Failed to start audio recording.")
                self.dictation_active = False
                if self.hotkey_manager.hotkey_active: self.hotkey_manager.hotkey_active = False
            self.update_menu_state()
